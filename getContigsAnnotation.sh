...
start_date=$(date)
 ...
 if [[ $(wc -l ${output_dir}OriginalUnmappedTags.txt|awk '{print $1}') -gt 0 ]];then
	...
	#print tag ID,chr,start,end,strand,nb_mismatch,gaps , LANG=en_EN sort  by mismaches, then gaps, then ID, then chromosome
...
#searching antisense tags, retain only one target (LANG=en_EN sort  by chr and start, then unique on attribute of the tag) (gives 2 cols in addition of the lineInSAM : Ensembl gene ID & HUGO ID)
