...
while getopts ":a:g:d:r:i:o:b:c:j:k:m:p:s:n:" opt; do
  case $opt in
...
getAssembliesPerLoci=$(dirname "$0")/getAssembliesPerLoci.R
...
if [ "$DEkupl_result" == "" ] || [ "$ref_fasta" == "" ] || [ "$diff_genes" == "" ] || [ "$ref_annotation" == "" ] || [ "$output_dir" == "" ] || [ "$illumina_adapters" == "" ] || [ "$samtools" == "" ] || [ "$bedtools" == "" ] || [ "$ncbi_blast_loc" == "" ] || [ "$GSNAP_loc" == "" ]; then
...
echo "threads number is : $threads"
...
start_date=$(date)
 ...
 if [[ $(wc -l ${output_dir}OriginalUnmappedTags.txt|awk '{print $1}') -gt 0 ]];then
	...
	if [ -f ${output_dir}UnmappedEntirelyFoundByBlast.tmp2 ];then
	  ...
	  awk 'OFS="\t"{if($7>$6){$6=$6-1;print $5,$6,$7,"LineInSam="$1";ID="$2";nb_hit="$13";nM="$14";del="$15";ins=0;clipped_5p=0;clipped_3p=0",1,$12,$6,$7,1,$7-$6}else{$7=$7-1;print $5,$7,$6,"LineInSam="$1";nb_hit="$13";nM="$14";del="$15";ins=0;clipped_5p=0;clipped_3p=0",1,$12,$7,$6,1,$6-$7,0}}' ${output_dir}UnmappedEntirelyFoundByBlast.tmp2 >>$diff_assemblies_bed
...
max_up_log2FC=$(awk 'NR>1{if($7 >0 || $7 <0){print $7}}' $DEkupl_result|awk '{if($1!~/-/){print}}' | sort -nr |head -n1)

#max value for downregulated features
max_down_log2FC=$(awk 'NR>1{if($7 >0 || $7 <0){print $7}}' $DEkupl_result|awk '{if($1~/-/){print}}'|sed 's/-//g'| sort -nr |head -n1)

#max absolute value
max_abs_log2FC=$(echo -e "${max_up_log2FC}\n${max_down_log2FC}"|sort -nr |head -n1)

#min value for upregulated features
min_up_log2FC=$(awk 'NR>1{if($7 >0 || $7 <0){print $7}}' $DEkupl_result|awk '{if($1!~/-/){print}}'| sort -n |head -n1)

#min value for downregulated features
min_down_log2FC=$(awk 'NR>1{if($7 >0 || $7 <0){print $7}}' $DEkupl_result|awk '{if($1~/-/){print}}'|sed 's/-//g'| sort -n |head -n1)
